...
def solution(id_list, report, k):
    ...
    # 신고 누적 계산
    for val in unique_report:
        users = val.split(" ")
        report_count[id_list.index(users[1])] += 1
    # 제재된 유저 리스트
    sanctions_users = []
    for idx, count in enumerate(report_count):
        if count >= k:
            sanctions_users.append(id_list[idx])
    # 제재된 유저를 신고한 유저에게 발송될 메일의 개수
    for val in unique_report:
        users = val.split(" ")
        if users[1] in sanctions_users:
            result[(id_list.index(users[0]))] += 1
